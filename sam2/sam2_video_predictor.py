...
class SAM2VideoPredictor(SAM2Base):
    ...
    def track(
        ...
        max_val = torch.max(video_res_masks)

        if max_val.item() == -1024:
            self.current_is_fully_occluded=True
        else:
            self.current_is_fully_occluded=False

        return obj_ids, video_res_masks, score, current_out

    '''

    def _manage_memory_obj(self, frame_idx, current_out, is_occluded=False, is_fully_occluded=False):
        output_dict = self.condition_state["output_dict"]

        # 初始化双层遮挡记忆
        occ_recent = output_dict.setdefault("occ_recent_mem", {})
        occ_distant = output_dict.setdefault("occ_distant_mem", {})

        if is_occluded:
            # 1️⃣ 部分遮挡：添加到 recent_occlusion memory
            if not is_fully_occluded:
                occ_recent[frame_idx] = current_out
                # 若 recent 满，则最早的帧转移至 distant memory
                if len(occ_recent) > self.num_occ_recent_mem:
                    oldest_t = sorted(occ_recent.keys())[0]
                    occ_distant[oldest_t] = occ_recent.pop(oldest_t)
                    # 保证 distant memory 不超限
                    if len(occ_distant) > self.num_occ_distant_mem:
                        for t in sorted(occ_distant.keys())[:-self.num_occ_distant_mem]:
                            occ_distant.pop(t, None)
            else:
                # 2️⃣ 完全遮挡：不更新 recent，保持 distant
                pass

        else:
            # 3️⃣ 非遮挡：正常更新 non-cond memory
            non_cond_outputs = output_dict["non_cond_frame_outputs"]
            non_cond_outputs[frame_idx] = current_out

            # 控制间隔存储
            keys_to_keep = []
            for t in sorted(non_cond_outputs.keys()):
                if t != frame_idx and t % self.memory_store_stride == 0:
                    keys_to_keep.append(t)
            keys_to_keep = keys_to_keep[-(self.num_maskmem - 1):]
            keys_to_keep.append(frame_idx)

            for t in list(non_cond_outputs.keys()):
                if t not in keys_to_keep:
                    non_cond_outputs.pop(t, None)

            # 若恢复检测正常，可清空 recent_occlusion memory
            if len(occ_recent) > 0:
                output_dict["occ_recent_mem"].clear()

    '''
    def _manage_memory_obj(self, frame_idx, current_out, is_occluded=False):
        output_dict = self.condition_state["output_dict"]

        if is_occluded:
            occ_outputs = output_dict["occ_frame_outputs"]
            occ_outputs[frame_idx] = current_out
            if len(occ_outputs) > self.num_occ_maskmem:
                for t in sorted(occ_outputs.keys())[:-self.num_occ_maskmem]:
                    occ_outputs.pop(t, None)
        else:
            non_cond_outputs = output_dict["non_cond_frame_outputs"]

            # 添加最近帧（frame_idx），无论是否满足stride
            non_cond_outputs[frame_idx] = current_out
            ...
            # 加上当前帧，总共最多保留 num_maskmem 个
            keys_to_keep = keys_to_keep[-(self.num_maskmem - 1):]  # 留 (N-1) 个间隔帧
            keys_to_keep.append(frame_idx)  # 加入当前帧
    ...
    def reset_state(self):
        ...
        for key in [
            ...
            if key not in self.condition_state:
                self.condition_state[key] = {} if "obj" in key else set()
    ...
    def _reset_tracking_results(self):
        ...
        # output_dict / consolidated_frame_inds 也要初始化
        if "output_dict" not in self.condition_state:
            self.condition_state["output_dict"] = {"cond_frame_outputs": [], "non_cond_frame_outputs": []}
        if "consolidated_frame_inds" not in self.condition_state:
            self.condition_state["consolidated_frame_inds"] = {"cond_frame_outputs": [], "non_cond_frame_outputs": []}
        ...
        self.condition_state["consolidated_frame_inds"]["non_cond_frame_outputs"].clear()

        self.condition_state["tracking_has_started"] = False
        if "frames_already_tracked" not in self.condition_state:
            self.condition_state["frames_already_tracked"] = set()
